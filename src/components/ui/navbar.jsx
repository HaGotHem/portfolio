...
import ProjetsDropdown from "@/components/ui/ProjetsDropdown";
...
const Navbar = () => {
  ...
  // Ferme le menu mobile au changement de page. Calculé pendant le rendu
  // (plutôt que dans un effet) pour éviter un rendu en cascade évitable.
  ...
  return (
    <header className="sticky top-4 z-50 w-full px-4 sm:px-6 md:px-8">
      <div className="mx-auto flex w-full max-w-10xl flex-col gap-3 md:flex-row md:items-center md:justify-between">
        ...
        <nav
          ref={containerRef}
          className="relative hidden w-full items-center gap-2 rounded-full p-2 md:flex md:w-auto"
        >
          {/* Le verre est une couche à part plutôt que le fond du <nav> : un
              backdrop-filter crée un "backdrop root" pour ses descendants, ce
              qui annulerait le flou du panneau déroulant imbriqué dedans. */}
          <div className="pointer-events-none absolute inset-0 rounded-full border border-white/25 bg-white/18 shadow-[0_10px_40px_rgba(0,0,0,0.30)] backdrop-blur-2xl backdrop-saturate-150" />
          {pillStyle && (
            <div
